...
#import "XLsn0wDispatchQueuePool.h"
...
@implementation XLsn0wDispatchQueuePool {
    @public
...
+ (instancetype)defaultPoolForQOS:(NSQualityOfService)qos {
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            static XLsn0wDispatchQueuePool *pool;
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[XLsn0wDispatchQueuePool alloc] initWithContext:YYDispatchContextGetForQOS(qos)];
            });
            return pool;
        } break;
        case NSQualityOfServiceUserInitiated: {
        ...
        default: {
            ...
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[XLsn0wDispatchQueuePool alloc] initWithContext:YYDispatchContextGetForQOS(NSQualityOfServiceDefault)];
            });
            return pool;
        } break;
    }
